var setup = function() {
  ...
  var setupEditor = function(id, json) {
    var text = "";
    if (json) { text = JSON.stringify(json, null, "    "); }
      ...
      config.postRender = function(form) {
        if (buildInteractionLayers) {
          ...
          form.getFieldEl().find(".alpaca-container-item").each(function() {
            ...
            $(buttonGroup).css({
              "margin-top": 8 + (($(interaction).height() / 2) - ($(buttonGroup).height() / 2)),
              "margin-right": "16px"
            });
          ...
          form.getFieldEl().find(".dropzone").droppable({
            ...
            "drop": function( event, ui ) {
              ...
              if (draggable.hasClass("form-element")) {
                ...
                insertField(
                  schema, options, data, dataType, fieldType, parentField,
                  previousField, previousFieldKey, nextField, nextFieldKey);
                ...
                form.moveItem(
                  draggedIndex, nextItemContainerIndex, false,
                  function() {
  ...
  var isCoreField = function(type) {
    var cores = [
      "any", "array", "checkbox", "file", "hidden", "number",
      "object", "radio", "select", "text", "textarea"];
  ...
  var insertField = function(schema, options, data, dataType, fieldType,
    ...
    parentField.addItem(
      itemKey, itemSchema, itemOptions,
      itemData, insertAfterId, function() {
